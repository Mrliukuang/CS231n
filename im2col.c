...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    float *A = (float *)mxGetData(M_IN);
    float *B;      // M_OUT data ptr
    ...
    // We prefer 'single' than 'double' to save memeory.
    //M_OUT = mxCreateDoubleMatrix(C*filter_h*filter_w, N*HH*WW, mxREAL);
    const mwSize out_size[] = {C*filter_h*filter_w, N*HH*WW};
    M_OUT = mxCreateNumericArray(2, out_size, mxSINGLE_CLASS, mxREAL);
    B = (float *)mxGetData(M_OUT);     // ptr B point to M_OUT data
